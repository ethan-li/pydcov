...
class TestBranchCoverage:
    ...
    def test_simple_branch_coverage(self):
        ...
        if (
            not shutil.which("gcov")
            or not shutil.which("lcov")
            or not shutil.which("genhtml")
        ):
...
"""
            ...
            result = subprocess.run(
                compile_cmd, capture_output=True, text=True, cwd=temp_path
            )
            ...
            result = subprocess.run(
                [str(exe_file)], capture_output=True, text=True, cwd=temp_path
            )
            ...
            print(
                f"Found {len(gcda_files)} .gcda files: {[f.name for f in gcda_files]}"
            )
            print(
                f"Found {len(gcno_files)} .gcno files: {[f.name for f in gcno_files]}"
            )
            ...
            result = subprocess.run(
                gcov_cmd, capture_output=True, text=True, cwd=temp_path
            )
            ...
            result = subprocess.run(
                geninfo_cmd, capture_output=True, text=True, cwd=temp_path
            )
            ...
            if has_brf:
                brf_lines = [
                    line for line in info_content.split("\n") if line.startswith("BRF:")
                ]
    ...
    def test_pydcov_branch_coverage_integration(self):
        ...
        if not build_dir.exists():
            pytest.skip(
                "Algorithm example not built - run: cd examples/algorithm/build && PYDCOV_ENABLE_COVERAGE=1 cmake .. && make"
            )
        ...
        if "PYDCOV_COVERAGE_ENABLED:BOOL=ON" not in cache_content:
            pytest.skip(
                "Coverage not enabled in CMake - run: cd examples/algorithm/build && PYDCOV_ENABLE_COVERAGE=1 cmake .."
            )
        ...
        result = subprocess.run(
            init_cmd, capture_output=True, text=True, cwd=algorithm_dir
        )
        ...
        result = subprocess.run(
            add_cmd, capture_output=True, text=True, cwd=algorithm_dir
        )
        ...
        if result.returncode != 0:
            ...
            print(
                "Warning: Test execution had issues, but continuing to check coverage data"
            )
        ...
        result = subprocess.run(
            merge_cmd, capture_output=True, text=True, cwd=algorithm_dir
        )
        ...
        if not merged_info.exists():
            pytest.skip(
                "No merged.info file generated - coverage data may not have been collected"
            )
        ...
        result = subprocess.run(
            report_cmd, capture_output=True, text=True, cwd=algorithm_dir
        )
