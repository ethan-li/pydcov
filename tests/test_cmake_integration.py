...
class TestCMakeHelperRunTarget:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = Mock(
                    returncode=0, stdout="Target completed successfully", stderr=""
                )
    ...
    def test_run_target_failure(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = Mock(
                    returncode=1, stdout="", stderr="Make target failed"
                )
...
install..."""
    ...
    def test_get_available_targets_failure(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = Mock(
                    returncode=1, stdout="", stderr="Make help failed"
                )
    ...
    def test_auto_detection_failure_scenarios(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with pytest.raises(
                    ValueError, match="Could not auto-detect CMake build directory"
                ):
...
class TestCMakeHelperEdgeCases:
    ...
    def test_run_target_with_output_logging(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = Mock(
                    returncode=0, stdout="Build output with details", stderr=""
                )
    ...
    def test_run_target_with_error_logging(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = Mock(
                    returncode=1, stdout="", stderr="Compilation failed with errors"
                )
    ...
    def test_get_available_targets_malformed_output(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = Mock(
                    returncode=0, stdout=malformed_output, stderr=""
                )
